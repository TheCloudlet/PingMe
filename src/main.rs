...
use cli_bridge::{
    ...
    MAX_SLACK_PHOTO_BYTES, PanePlacement, SessionStatus, SlackFile, ThreadAction, ThreadMessage,
    accepted_slack_photos, agent_display_name, apply_host_routing_policy, control_action,
    pane_placement, prompt_with_local_photos, render_session_status_card, self_test_message,
    session_update_after_prompt_paste, slack_file_url_is_downloadable, slack_photo_store_path,
};
...
fn main() -> ExitCode {
    ...
    let bot_token = env_values
        ...
        .cloned()
        .unwrap_or_default();
    let mut services = RealServices {
        bot_token,
        current_pane_agent: None,
    };
    let result =
        cli_bridge::run_cli_with_services(&arg_refs, &setting_toml, &env_refs, &mut services);
...
impl LocalServices for RealServices {
    ...
    fn create_session_thread(&mut self, session: AgentSession) -> Result<AgentSession, String> {
        ...
        Ok(session.with_thread(ts, None))
    }

    fn update_session_thread(&mut self, session: &AgentSession) -> Result<(), String> {
        let root_ts = session
            .thread_ts()
            .ok_or_else(|| "Agent Session has no Session Thread".to_owned())?;
        let response: Value = reqwest::blocking::Client::new()
    ...
    fn spawn_agent(&mut self, session: AgentSession) -> Result<AgentSession, String> {
        ...
        if session.placement() == Some(PanePlacement::ReuseCurrentPane) {
            let pane_id = session.require_pane()?.to_owned();
            self.current_pane_agent = Some(CurrentPaneAgent {
                command: current_pane_shell_command(
                    &pane_id,
                    &agent_command(&session, session.notify_socket(), session.notify_token()),
                ),
    ...
    fn register_session(&mut self, session: &AgentSession) -> Result<(), String> {
        ...
        if let Some(token) = session.notify_token() {
        ...
        if session.is_self_test() {
            ...
            if let Some(agent) = &mut self.current_pane_agent {
                agent.self_test = Some(CurrentPaneSelfTest {
                    control_channel_id: session.control_channel().to_owned(),
                    thread_ts: session.require_thread()?.to_owned(),
    ...
    fn attach_session(&mut self, session_name: &str) -> Result<(), String> {
        ...
        if env::var_os("TMUX").is_some() {
            tmux_status(&["select-window", "-t", session.require_tmux_window()?])?;
            tmux_status(&["select-pane", "-t", session.require_pane()?])
        } else {
            tmux_status(&["select-pane", "-t", session.require_pane()?])?;
            tmux_status(&["select-window", "-t", session.require_tmux_window()?])?;
            attach_tmux_session(session.require_tmux_session()?)
        }
    }
...
fn run_notify_process(args: &[String]) -> ExitCode {
...
fn run_daemon_process(args: &[String]) -> ExitCode {
...
fn ended_agent_sessions(known: &[AgentSession], current: &[AgentSession]) -> Vec<AgentSession> {
    let mut ended: Vec<AgentSession> = known
        ...
        .filter(|known| {
            known.is_available()
                && !known.is_reported_unavailable()
                && current
                    .iter()
                    .find(|current| current.pane_id() == known.pane_id())
                    .is_none_or(|current| !current.is_available())
        })
        .cloned()
        .collect();
    ended.extend(
        current
            ...
            .filter(|current| {
                !current.is_available()
                    && !current.is_reported_unavailable()
                    && !known.iter().any(|known| {
                        known.pane_id() == current.pane_id()
                            && (known.is_available() || known.is_reported_unavailable())
                    })
            })
            .cloned(),
    );
...
fn pane_is_registered(sessions: &[AgentSession], pane_id: &str) -> bool {
    sessions
        .iter()
        .any(|session| session.pane_id() == Some(pane_id))
}
...
async fn monitor_agent_sessions(
    ...
    for ended_session in &ended {
        ...
        if pane_is_registered(&current, session.require_pane()?) {
...
async fn launch_remote_agent(
    ...
    bot_token: &str,
    setting: &HostSettings,
    project: &cli_bridge::ProjectSetting,
    agent_cli: &str,
    ...
    Ok(RemoteLaunch {
        session_name: started.session.name().to_owned(),
        warning,
    })
}
...
async fn handle_slack_event(
    ...
    match action {
        ThreadAction::Prompt(prompt) => {
            if !pane_available(config.session.require_pane()?)? {
            ...
            if let Err(error) = paste_into_pane(config.session.require_pane()?, &prompt) {
                let _ = config.session.write_last_slack_prompt(None);
                config.session.write_status(SessionStatus::Idle)?;
        ...
        ThreadAction::Stop => {
            ...
            tmux_status(&["send-keys", "-t", config.session.require_pane()?, "Escape"])?;
            config.session.write_status(SessionStatus::Idle)?;
        ...
        ThreadAction::Name => {
            post_slack(
                http,
                &config,
                &format!("Session Name: {}", config.session.name()),
            )
            .await?;
        }
...
async fn handle_agent_notification(
    ...
    if notification.get("token").and_then(Value::as_str) != config.session.notify_token() {
        return Ok(());
    }
    ...
    let terminal_prompts = if config.session.agent_cli() == "codex" {
        terminal_prompts(&notification, config.session.last_slack_prompt())
    } else {
        Vec::new()
    };
    ...
    if write_self_test_status(
        config.session.is_self_test(),
        config.session.status_file().map(std::path::Path::new),
        response,
    )? {
...
async fn materialize_thread_prompt(
    ...
    for photo in photos {
        let dest = slack_photo_store_path(config.session.require_thread()?, photo)?;
...
async fn post_slack(
    ...
    let thread_ts = config.session.require_thread()?;
    let response: Value = http
        .post("https://slack.com/api/chat.postMessage")
...
fn cleanup_agent_sessions(bot_token: &str) -> Result<usize, String> {
    ...
    cleanup_unavailable_sessions(
        ...
        |session| tmux_status(&["kill-window", "-t", session.require_tmux_window()?]),
    )
}
...
async fn rename_session(
    ...
    tmux_status(&["rename-window", "-t", config.session.require_pane()?, name])?;
    config.session.write_name(name)?;
    paste_into_pane(config.session.require_pane()?, &format!("/rename {name}"))?;
    update_slack_root(http, config).await
}
...
fn spawn_agent_tui(session: AgentSession) -> Result<AgentSession, String> {
    ...
    let notify_socket = session
        ...
        .transpose()?;
    let notify_token = session.thread_ts().map(|thread_ts| {
        format!(
            "{}-{}-{thread_ts}",
            std::process::id(),
            std::time::SystemTime::now()
...
fn attach_or_switch_agent_tui(
    session: &AgentSession,
    services: &mut RealServices,
) -> Result<(), String> {
    match session.placement() {
        Some(PanePlacement::DetachedSession { attach: true }) => {
            attach_tmux_session(session.tmux_session().unwrap_or(session.name()))
        }
...
fn grok_command(
    ...
    match (session.thread_ts(), notify_socket_path, notify_token) {
...
mod tests {
    ...
    fn test_session(
        ...
        let session = match thread_ts {
            Some(thread_ts) => session.with_thread(thread_ts, None),
            None => session,
        };
        let mut session = session.with_spawned(
            pane_id,
            Some("bridge".to_owned()),
            Some("bridge:3".to_owned()),
            Some("/tmp/bridge.sock".to_owned()),
    ...
    fn recorded_cli_stop_decides_to_stop_without_ending_the_session() {
        ...
        assert!(routed.session.is_available());
        ...
        publish_ended_agent_session(
            ...
            |session| {
                events.borrow_mut().push(format!(
                    "tui-ended:{}:{}",
                    session.control_channel(),
                    session.thread_ts().unwrap_or_default()
                ));
                Ok(())
            },
            |session| {
    ...
    fn cleanup_deletes_session_thread_root_before_local_mapping() {
        ...
        let cleaned = cleanup_unavailable_sessions(
            ...
            |session| {
                events.borrow_mut().push(format!(
                    "remove-mapping:{}",
                    session.tmux_window().unwrap_or_default()
                ));
                Ok(())
            },
        )
    ...
    fn cleanup_accepts_an_already_missing_session_thread_root() {
        ...
        let cleaned = cleanup_unavailable_sessions(
            ...
            |session| {
                removed
                    .borrow_mut()
                    .push(session.tmux_window().unwrap_or_default().to_owned());
                Ok(())
            },
        )
        .unwrap();
    ...
    fn pane_monitor_detects_an_ended_agent_session_once() {
        ...
        assert!(!pane_is_registered(&[], live.pane_id().unwrap_or_default()));
        assert!(pane_is_registered(
            std::slice::from_ref(&unavailable),
            live.pane_id().unwrap_or_default()
        ));
        assert!(
    ...
    fn pane_monitor_records_each_success_before_the_next_publication() {
        ...
        assert!(!known[0].is_available());
        assert_eq!(SessionStatus::Unavailable, known[0].status());
        assert!(known[1].is_available());
    }
